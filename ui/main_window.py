...
#-------
    ...
    def _on_checkbox_change(self, name, var):
        if name == "Loaned out":
            ...
            date_var = self.location_vars.get("Loaned out date") or self.reg_vars.get("Loaned out date")
            if date_var:
                if val in ("true", "1"):
                    date_var.set(datetime.now().strftime("%d.%m.%Y %H:%M:%S"))
                else:
                    date_var.set("")
        self.commit_current_object()
...
#--------- Books -------------
    ...
    def open_focus_settings(self):
        ...
        ttk.Button(btn_row, text="Cancel", command=_close_focus_win, width=10, style="Tool.TButton").pack(side="right", padx=4)
