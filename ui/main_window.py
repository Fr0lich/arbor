...
class ToggleSwitch(tk.Canvas):
    def __init__(self, parent, variable, command=None, width=42, height=22, ui_ref=None, **kwargs):
        # Scale the size using config.sc() if ui_ref is available
    ...
    def draw(self):
        ...
        # Detect light/dark mode and parent frame background
        is_dark = False
        ...
        if is_dark:
            bg_canvas = "#1e1e2e"  # background matching dark mode main window
            bg_active = "#a6e3a1"  # Catppuccin green
            bg_inactive = "#313244"
            fg_knob_active = "#11111b"
            fg_knob_inactive = "#cdd6f4"
        else:
            bg_canvas = "#f3f3f3"  # surface-container-low (#f3f3f3)
            bg_active = "#3b6934"  # secondary green (#3b6934)
            bg_inactive = "#c4c7c7" # outline-variant (#c4c7c7)
            fg_knob_active = "#ffffff"
            fg_knob_inactive = "#ffffff"
            
        self.configure(bg=bg_canvas)
        
        # Draw switch track (sharp flat rectangle)
        self.create_rectangle(0, 0, w, h, fill=bg_active if val else bg_inactive, outline="")
        
        # Draw sliding knob (sharp flat square)
        pad = 2
        knob_size = h - pad * 2
        if val:
            x = w - pad - knob_size
...
class TreeviewListboxWrapper(ttk.Treeview):
    ...
    def _on_treeview_click(self, event):
        ...
        if region == "cell":
            ...
            if column == "#1":  # The "Rev" column
                item_id = self.identify_row(event.y)
                if item_id:
                    self.main_window._toggle_reviewed_for_id(item_id)
                    # Force selection change immediately
